...
int main(int argc, char **argv)
{
    ...
    std::cout << "Data Count:     " << tree.nodeSize() << std::endl;
    std::cout << "Tree Depth:     " << tree.levelSize() << std::endl;
    std::cout << std::fixed << std::setprecision(2);
    std::cout << "Memory Use:     " << human_readable(tree.byteUseSize()) << std::endl;
#ifdef BLOOMFILTER
    ...
    /* for (auto d : data)
        assert(tree.isExist(d));*/

    std::vector<std::string *> test_data;
    std::vector<size_t> test_int;
    std::vector<bool> ans; //ans用来记录每个数据是正确还是错误
    ...
    for (size_t i = 0; i < TEST_SIZE; ++i)
        assert(tree.isExist(test_data[i]) == ans[i]);
